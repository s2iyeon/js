...
- [3. use strict](#3-use-strict)
  + [3.1 use strict란?](#31-use-strict란)
  + [3.2 엄격 모드에서 적용되는 주요 문법](#32-엄격-모드에서-적용되는-주요-문법)
- [4. 동기 방식과 비동기 방식](#4-동기-방식과-비동기-방식)
  + [4.1 script 태그의 속성 async, defer](#41-script-태그의-속성-async-defer)
  + [4.2 일반 함수 (동기 방식)](#42-일반-함수-동기-방식)
  + [4.3 비동기 함수](#43-비동기-함수)
  + [4.4 비동기 함수의 결과 처리 - 1. 콜백 방식](#44-비동기-함수의-결과-처리---1-콜백-방식)
  + [4.5 비동기 함수의 결과 처리 - 2. Promise](#45-비동기-함수의-결과-처리---2-promise)
  + [4.6 비동기 함수의 결과 처리 - 3. async/await](#46-비동기-함수의-결과-처리---3-asyncawait)
...
에러 발생
Error: 에러 발생
    at f1 (C:\febc15\js\workspace-ins\ch08\ex08-01.js:4:21)
    at f2 (C:\febc15\js\workspace-ins\ch08\ex08-01.js:10:9)
    at Object.<anonymous> (C:\febc15\js\workspace-ins\ch08\ex08-01.js:12:5)
    ......
```
...
- 선택적으로 finally 블럭을 추가하면 try 블럭이나 catch 블럭의 코드가 실행된 후 실행
...
function f1() {
  try {
    ...
    console.log(2, fn(10, 20)); // 실행 안됨
  } catch(err) {
    // catch에 전달되는 에러는 unknown 타입이기 때문에 타입 가드 필요
    if(err instanceof Error) {
      console.error('3. 에러 발생.', err.message);
    }
  } finally {
    console.log('4. try...catch 문의 마지막에 호출');
  }
  console.log('5. f1 함수 정상 종료'); // 에러가 발생해도 실행됨
}
f1();
```
...
```
1. f1 호출
3. 에러 발생. Unexpected identifier 'x'
4. try...catch 문의 마지막에 호출
5. f1 함수 정상 종료
```
...
- 에러를 직접 처리하지 않고 함수를 호출한 쪽으로 전달할 때 사용
- 에러가 throw 되면 프로그램 실행이 중지되고 가까운 catch 블럭으로 이동
- throw 되지 않은 에러는 catch 블럭으로 전달되지 않고 프로그램이 비정상 종료됨

#### throw 문법
- `throw 표현식;`
- 표현식에는 Error 객체, 문자열, 숫자 등 모든 값을 사용할 수 있지만, 일반적으로 Error 객체나 Error를 상속받은 객체를 사용하는 것이 권장됨

#### throw할 수 있는 값
- Error 객체: `throw new Error('에러 메시지')`
- 사용자 정의 에러 객체: `throw new CustomError('에러 메시지')`
- 문자열, 숫자 등 원시값도 가능하지만 권장하지 않음
...
#### 에러 전파
- throw된 에러는 호출 체인을 따라 상위 함수로 전파되며, 가장 가까운 catch 블럭에서 처리됨
- catch 블럭이 없으면 프로그램이 비정상 종료됨
- 중첩된 함수에서 throw된 에러도 상위 호출자의 catch 블럭에서 처리 가능

#### 사용 시나리오
- 함수 내에서 잘못된 입력값이나 조건을 검사하여 내부 코드의 실행에 문제가 있을 때 에러 발생
- 비즈니스 로직에서 허용되지 않은 동작을 방지
- 에러 처리를 상위 함수로 위임하여 책임 분리
...
#### Dynamic Import
- 런타임에 조건부로 모듈을 동적으로 가져오는 방법
- `import()` 함수를 사용하며 Promise를 반환
- 정적 import와 달리 모듈 경로를 문자열로 동적으로 지정 가능
- 조건부 로딩, 코드 분할 등의 기법에 사용하며 성능 최적화에 도움
- async/await 또는 Promise의 then()을 사용하여 처리
...
- ES6에 도입된 자바스크립트의 공식 모듈 표준
- 내보내기: export 키워드 사용
- 가져오기: import 키워드 사용

#### CJS (CommonJS)
- CommonJS: 브라우저 밖에서 실행되는 자바스크립트를 위한 명세서
- Node.js에서 모듈화를 위해 사용하는 기본 방식
...
- `.cjs`: `"type": "module"` 속성의 유무와 상관 없이 CJS 방식의 모듈 사용
- `.mjs`: `"type": "module"` 속성의 유무와 상관 없이 ESM 방식의 모듈 사용
...
- 자바스크립트의 문제점으로 지적되어온 몇가지 문법들은 새로운 버전이 나오면서 보완 되었지만 하위 호환을 위해서 새로운 문법들이 비활성화 되어 있음
...
### 3.2 엄격 모드에서 적용되는 주요 문법

- 변수를 선언 없이 사용하면 오류 발생
- 함수를 직접 호출할때 this가 window가 아닌 undefined가 됨
- 중복 매개변수 사용 금지
...
<img src="images/08-02.png" width="640">
...
<img src="images/08-03.png" width="640">
...
#### 💻 [08-07 script 태그의 속성 async, defer](../workspace-ins/ch08/ex08-07/)
...
#### 💻 [08-08 일반 함수(동기 방식)](../workspace-ins/ch08/ex08-08.js)
...
#### 💻 [08-09 비동기 함수](../workspace-ins/ch08/ex08-09.js)

---

### 4.4 비동기 함수의 결과 처리 - 1. 콜백 방식
...
#### 💻 [08-09-02 비동기 함수 - 콜백 함수 사용](../workspace-ins/ch08/ex08-09-02.js)

#### 💻 [08-09-03 비동기 함수 - 콜백 함수에 인자값 전달](../workspace-ins/ch08/ex08-09-03.js)

#### 💻 [08-09-04 비동기 함수 - 결과의 순서가 보장되지 않음](../workspace-ins/ch08/ex08-09-04.js)

#### 💻 [08-09-05 비동기 함수 - 결과의 순서 보장](../workspace-ins/ch08/ex08-09-05.js)

---

### 4.5 비동기 함수의 결과 처리 - 2. Promise

#### Promise란?
- 비동기 작업의 성공 또는 실패 결과를 나중에 전달하기 위한 객체(ES6에 추가)
...
- `<T>`: resolve 함수 인자값의 타입

#### Promise의 상태
- Promise 객체는 세 가지 상태 중 하나를 가짐
- pending (대기): 초기 상태. executor 함수가 실행 중이며 아직 resolve()나 reject()가 호출되지 않은 상태
- fulfilled (이행): 비동기 작업이 성공적으로 완료된 상태. resolve()가 호출되어 값을 전달받은 상태
- rejected (거부): 비동기 작업이 실패한 상태. reject()가 호출되어 에러가 발생한 상태
- settled (완결): pending 상태에서 fulfilled 또는 rejected로 최종 상태가 결정된 상태
- Promise의 상태는 한 번 변경되면 더 이상 변경되지 않음 (immutable)

#### Promise를 이용한 비동기 함수 작성
- Promise 객체 반환
  + Promise 객체를 반환하는 함수는 비동기 함수가 됨
- Promise 생성자 함수의 `executor` 함수 작성
  + `executor` 함수: 처리할 작업 구현
...
##### 💻 [08-10 비동기 함수 - Promise 사용](../workspace-ins/ch08/ex08-10.js)

##### 💻 [08-10-02 Promise - 에러 처리 추가](../workspace-ins/ch08/ex08-10-02.js)
...
##### 💻 [08-10-03 Promise - 체인 방식으로 호출 1](../workspace-ins/ch08/ex08-10-03.js)

##### 💻 [08-10-04 Promise - 체인 방식으로 호출 2](../workspace-ins/ch08/ex08-10-04.js)
...
#### 💻 [08-10-05 Promise의 static 메서드](../workspace-ins/ch08/ex08-10-05.js)

---

### 4.6 비동기 함수의 결과 처리 - 3. async/await
...
async function a2(){ // async
  throw 'a2 에러'; // Promise.reject('a2 에러')와 동일
}
```

#### 💻 [08-11 async](../workspace-ins/ch08/ex08-11.js)
...
#### 💻 [08-11-02 await](../workspace-ins/ch08/ex08-11-02.js)

#### 💻 [08-12 async/await](../workspace-ins/ch08/ex08-12.js)
