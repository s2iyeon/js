...
```
- HTTP 버전: HTTP/1.1 또는 HTTP/2 등
- 상태 코드: 200(성공), 404(찾을 수 없음), 500(서버 에러) 등
- 상태 메시지: OK, Created, Not Found 등 응답 상태를 간단한 텍스트로 표현
...
##### 예시
- GET `/todolist`: 할일 목록 조회
- POST `/todolist`: 새 할일 등록
- PATCH `/todolist/5`: 할일 정보 수정 (완료 여부, 내용 등)
- DELETE `/todolist/5`: 할일 삭제
